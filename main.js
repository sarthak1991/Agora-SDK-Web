...
console.log(config.VITE_testVar);
  ...
  if (!isRecording) {
    try {
      ...
      if (data) {
        ...
        recordingData = await startWebRecording(
          CONSTANTS.APPID,
          acquisitionData.data.resourceId,
          acquisitionData.data.cname,
          acquisitionData.data.uid,
    ...
    try {
      if (recordingData) {
       ...
       const dataStopped =  await stopWebRecording(
          CONSTANTS.APPID,
          recordingData.data.resourceId,
          recordingData.data.sid,
          CONSTANTS.CHANNEL,
          recordingData.data.uid
        );

        console.log("This is the recording data =-=-=-=-=-=--=-=-=-=-=");
        console.log("This is the recording data =-=-=-=-=-=--=-=-=-=-=");
        console.log("This is the recording data =-=-=-=-=-=--=-=-=-=-=");
...
const toggleRTS = async (e) => {
    ...
    if (!isRTSRunning) {
        try {
            ...
            let builderToken = acquireResult.data.tokenName;
            ...
            rtsData.builderToken = acquireResult.data.tokenName;
...
// Any changes you wanna makem you can make after this comment. 
